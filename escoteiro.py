...
def obter_copy(tema):
    copys = {
        "Score 900 Turbo": "O Guia Secreto para dominar o sistema bancário. Aprenda como os algoritmos de crédito te avaliam e ative os gatilhos ocultos para subir sua pontuação em tempo recorde.",
        "Renda Extra": "Transforme seu dispositivo móvel em uma fonte de renda passiva e ativa. Métodos validados de arbitragem digital e micro-tarefas de alta remuneração.",
        "Planilha Lucros": "Gestão financeira com inteligência artificial aplicada. Visualize cada centavo, projete lucros futuros e identifique gargalos de gastos automaticamente."
    }
...
def gerar_layout_pagina(tema, preco, link, eh_detalhes=False, relacionados=[]):
    ...
    # HTML dos 3 Produtos Relacionados (Restaurado)
    html_relacionados = ""
    if relacionados:
        for p in relacionados:
            ...
            <a href="{p['slug']}.html" class="glass p-8 block hover:bg-white/10 transition-all text-center border border-white/5">
                ...
                <span class="text-[10px] uppercase opacity-50 mt-4 block">Ver Protocolo</span>
            </a>'''
    ...
    <div class="mt-16 space-y-12">
        ...
        <div class="glass p-10">
            <h3 class="titulo-master text-2xl mb-6 italic">O Método:</h3>
            <p class="text-slate-300 text-lg leading-relaxed mb-8">{copy_detalhada}</p>
        </div>
        <div class="glass p-12 text-center neon-border">
            ...
            <div class="mt-8 flex items-center justify-center gap-4 text-sm text-green-400">
                ...
                <p class="text-left font-bold">Garantia Incondicional de 7 Dias<br><span class="text-white opacity-50">Risco Zero</span></p>
            </div>
        </div>
    </div>""" if eh_detalhes else ""
    ...
    <body>
        ...
        <div class="max-w-5xl mx-auto px-6 py-20">
            <header class="flex justify-between items-center mb-20">
                ...
                <div class="text-[10px] font-bold uppercase tracking-widest opacity-50">Intelligence Scraper v4.0</div>
            </header>
            {conteudo_venda if eh_detalhes else f'<h1 class="titulo-master text-7xl md:text-9xl mb-12 italic">#{tema}</h1><a href="detalhes.html" class="btn-venda py-6 text-xl">ACESSAR SISTEMA</a>'}
            {secao_relacionados}
